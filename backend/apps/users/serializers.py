...
class UserSignUpSerializer(serializers.ModelSerializer):
    ...
    token_expires = serializers.CharField(write_only=True)
...
class UserSignInSerializer(serializers.ModelSerializer):
    ...
    def create(self, validated_data):
        ...
        if len(user) > 0 and check_password(validated_data['password'], user[0].password):
            ...
            user[0].token_expires = datetime.datetime.now() + datetime.timedelta(days=5)
            user[0].save()

            return user[0]
        else:
            raise serializers.ValidationError({"error": "The login information is incorrect"})
